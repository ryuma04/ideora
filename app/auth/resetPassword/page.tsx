...
function ResetPasswordForm() {
    ...
    const token = params.get("token");//extracting the token
    ...
    const send = async () => {
        try {
            console.log("Token:", token);//for testing
            console.log("Password", newpassword);//for testing
            const res = await fetch("/api/auth/resetPassword", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
                body: JSON.stringify({ token, password: newpassword })//sending the password and the token to the backend
            })
